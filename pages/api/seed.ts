...
export default async function handler(
	...
	const vivaanId =
		restaurants.find(
			(restaurant) => restaurant.name === "Vivaan - fine Indian"
		)?.id || 1;
	const RamaKrishnaId =
	...
	const lastTrainToDelhiId =
		restaurants.find(
			(restaurant) => restaurant.name === "Last Train to Delhi"
		)?.id || 1;
	...
	const eldoradoTacoId =
		restaurants.find((restaurant) => restaurant.name === "Eldorado Taco")
			?.id || 1;
	const laBartolaId =
	...
	await prisma.item.createMany({
		data: [
			...
			},
			{
				...
				description:
					"Cured scallop served with mangosteen and coconut broth",
				price: "$18.00",
				restaurant_id: vivaanId,
			},
			{
			...
			},
			{
				...
				description:
					"Turmeric lemon butter sauce, curry leaves, mustard seeds",
				price: "$39.00",
				restaurant_id: curryishTavernId,
			},
			{
			...
			},
			{
				name: "Lamb Lollipops",
				description:
					"grilled chops with turmeric, mint and fenugreek curry",
				price: "$44.00",
			...
			},
			{
				...
				description:
					"tofu, sweet peppers, red onion, tomato and cashew cream",
				price: "$23.00",
				restaurant_id: pukkaId,
			},
			{
			...
			},
			{
				...
				description:
					"Scallops, mussels, shrimp and crab meat in a rose sauce",
				price: "$33.00",
	...
	const userLaith = await prisma.user.create({
		data: {
			...
			city: "ottawa",
			password:
				"$2b$10$I8xkU2nQ8EAHuVOdbMy9YO/.rSU3584Y.H4LrpIujGNDtmny9FnLu",
			phone: "1112223333",
		},
	});
	...
	await prisma.review.createMany({
		data: [
			...
			},
			{
				first_name: "Laith",
				last_name: "Harb",
				text: "Restaurant was loud and crowded. Food is not worth the ITEM_Price.",
				rating: 3,
				restaurant_id: eldoradoTacoId,
				user_id: userLaith.id,
			},
